...
# Go to the temp foler located at the data folder. Copy the data from the subjects to it.
	...
	for j in $(eval echo "{0..$NumSub}"); do
		SubID=${subjects[$j]}
		scp -r $data/$SubID/IMAGEN_con_0006_EPI_faces_$SubID.nii .
		scp -r $data/$SubID/IMAGEN_varcon_0006_EPI_faces_$SubID.nii .
		# Copy the universal mask to the temp folder, rename it with a subject index and then send it to the GROUPWD
		scp -r $data/AllSubMaskFaces.nii .
		mv AllSubMaskFaces.nii AllSubMaskFaces_$SubID.nii
		mv AllSubMaskFaces_$SubID.nii "$GROUPWD/"
		# Array with ConFile and VarConFile
		ConFile=("${ConFile[@]}" IMAGEN_con_0006_EPI_faces_$SubID.nii)
		VarConFile=("${VarConFile[@]}" IMAGEN_varcon_0006_EPI_faces_$SubID.nii)
	done
	...
	$FSLDIR/bin/fslmerge -t "${GROUPWD}"/mask.nii AllSubMaskFaces*.nii
	
	# Remove individual masks
	rm -r AllSubMaskFaces*.nii
...
scp -r $data/33/IMAGEN_con_0006_EPI_faces_33.nii .
$FSLDIR/bin/fslcpgeom IMAGEN_con_0006_EPI_faces_33.nii thresh_zstat1.nii
# Also for tstat1.nii, because we use it later on in transformations
		cd stats
		scp -r $data/33/IMAGEN_con_0006_EPI_faces_33.nii .
		$FSLDIR/bin/fslcpgeom IMAGEN_con_0006_EPI_faces_33.nii tstat1.nii.gz
		$FSLDIR/bin/fslcpgeom IMAGEN_con_0006_EPI_faces_33.nii zstat1.nii.gz
		rm -r IMAGEN_con_0006_EPI_faces_33.nii
		cd ..
rm -r IMAGEN_con_0006_EPI_faces_33.nii
