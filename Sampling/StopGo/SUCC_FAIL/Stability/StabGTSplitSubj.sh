...
# Go to the temp foler located at the data folder. Copy the data from the subjects to it.
	...
	for j in $(eval echo "{0..$NumSub}"); do
		SubID=${subjects[$j]}
		# Copy contrast map
		scp -r $data/$SubID/EPI_stop_signal/con_0005_stop_success_-_stop_failure.nii.gz .
		# Rename with subject ID (otherwise we have duplicates in the temp folder)
		mv con_0005_stop_success_-_stop_failure.nii.gz con_0005_stop_success_-_stop_failure_$SubID.nii.gz
		# Extract
		gunzip con_0005_stop_success_-_stop_failure_$SubID.nii.gz
		# Same with variance
		scp -r $data/$SubID/EPI_stop_signal/varcon_0005_stop_success_-_stop_failure.nii.gz .
		mv varcon_0005_stop_success_-_stop_failure.nii.gz varcon_0005_stop_success_-_stop_failure_$SubID.nii.gz
		gunzip varcon_0005_stop_success_-_stop_failure_$SubID.nii.gz
		...
		ConFile=("${ConFile[@]}" con_0005_stop_success_-_stop_failure_$SubID.nii)
		VarConFile=("${VarConFile[@]}" varcon_0005_stop_success_-_stop_failure_$SubID.nii)
	done
...
scp $data/33/EPI_stop_signal/con_0005_stop_success_-_stop_failure.nii.gz .
gunzip con_0005_stop_success_-_stop_failure.nii.gz
$FSLDIR/bin/fslcpgeom con_0005_stop_success_-_stop_failure.nii thresh_zstat1.nii
# Same on nifti file with the clusters
$FSLDIR/bin/fslcpgeom con_0005_stop_success_-_stop_failure.nii clusteroutput.nii.gz
# Also for tstat1.nii, because we use it later on in transformations
		cd stats
		scp -r $data/33/EPI_stop_signal/con_0005_stop_success_-_stop_failure.nii.gz .
		gunzip con_0005_stop_success_-_stop_failure.nii.gz
		$FSLDIR/bin/fslcpgeom con_0005_stop_success_-_stop_failure.nii tstat1.nii.gz
		$FSLDIR/bin/fslcpgeom con_0005_stop_success_-_stop_failure.nii zstat1.nii.gz
		rm con_0005_stop_success_-_stop_failure.nii
		cd ..
rm con_0005_stop_success_-_stop_failure.nii
