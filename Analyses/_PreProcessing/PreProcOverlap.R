...
## Save R objects
saveRDS(MatrixOverlap, 
        file = paste(SaveLoc, '/', contr ,'/MaitraOverlap.rda',sep=''))
saveRDS(PercAct,
        file = paste(SaveLoc, '/', contr, '/PercActMaitraOverlap.rda',sep=''))
...
FDR_Ts <- c(0.2, 0.1, 0.01, 0.001)

# Start for loop over the thresholds
for(t in 1:length(FDR_Ts)){
  # Select the corresponding FDR threshold from the vector
  FDR_Pth <- FDR_Ts[t]
  # Print statement
  print(paste('-------------- AT THRESHOLD FDR ', FDR_Pth, '. PLEASE STAY CALM! --------------', sep = ''))
  ...
  ## Save R objects
  saveRDS(MatrixOverlapFDR, 
    file = paste(SaveLoc, '/', contr, '/MaitraOverlapFDR', IDTh, '.rda',sep=''))
}
