####################
#### TITLE:     Adaptive thresholding: analysis in which significance thresholding level is adapted in order to keep percentage of voxels at constant level. OLS pooling.
#### Contents: 	
#### 
...
## The data comes from scenario 6: based on OLS pooling!
...
# Set WD
wd <- "/Volumes/2_TB_WD_Elements_10B8_Han/PhD/IMAGENDATA/Data/FreddieFreeloader/Data/OLS/Scenario9"
setwd(wd)
...
dat <- '/Volumes/2_TB_WD_Elements_10B8_Han/PhD/IMAGENDATA/Data/FreddieFreeloader/Data/OLS/Scenario6'
...
for(i in 1:NRUNS){
	...
	for(j in 1:NSTEP){
		...
		###################################################################
		# Now that we have t-statistic images with its masks, calculate p-values using baseline significance level
			# Degrees of freedom
			df <- DF[j]
		pvalsG1 <- 1-pt(imageG1,df)
		pvalsG2 <- 1-pt(imageG2,df)
	
		# Function AdapThresholding: calculates p-values, then finds correct significance level according to percentage.
...
## Save objects
save(MatrixOverlapAdap,file=paste(wd,'/MatrixOverlapAdapScen9',sep=''))
save(PercActAdap,file=paste(wd,'/PercActAdapScen9',sep=''))
save(SignLevels,file=paste(wd,'/SignLevelsAdapScen9',sep=''))

# Or load them in
load(paste(wd,'/MatrixOverlapAdapScen9',sep=''))
load(paste(wd,'/PercActAdapScen9',sep=''))
load(paste(wd,'/SignLevelsAdapScen9',sep=''))
...
#################
	...
	SignLevels <- data.frame(	'Value' = sign,
								'Size' = sampleSize,
								'Type' = rep('SignLevel', length(perc)))
...
prevWD <- "/Volumes/2_TB_WD_Elements_10B8_Han/PhD/IMAGENDATA/Data/FreddieFreeloader/Data/SplitSubjects/ScenarioA"
load(paste(prevWD,'/MatrixOverlapSplitSubjA',sep=''))
...
ggplot(AvgOverPercSignPrev, aes(x=factor(Size),y=Value,group=Type)) + 
	geom_line(aes(colour=Type),size=1.3) +
	scale_colour_manual(values=colours,labels=c('Overlap','Percentage','Significance level', 'Previous overlap')) +
	scale_x_discrete(breaks=subjBreak, name="Sample size") +
	scale_y_continuous(name='Value') +
	theme(plot.title = element_text(lineheight=.2, face="bold")) +
