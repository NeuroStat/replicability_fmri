####################
#### TITLE:     Calculate within sample size stability with increasing sample sizes: scenario C. OLS - POOLING
#### Contents: 	
#### 
...
# Pooling of subjects based on FSL's OLS
...
# Set WD
wd <- "/Volumes/2_TB_WD_Elements_10B8_Han/PhD/IMAGENDATA/Data/FreddieFreeloader/Data/OLS/Scenario8"
setwd(wd)
...
## Save objects
save(MatrixOverlap,file=paste(wd,'/MatrixOverlapOLSScen8',sep=''))
save(PercAct,file=paste(wd,'/PercActOLSScen8',sep=''))
...
Overlap.tmp <- matrix(MatrixOverlap,ncol=1)
	sampleSize <- rep(seq(10,70,by=10),NRUNS)
Overlap <- data.frame('overlap' = Overlap.tmp, 'size' = sampleSize)

# Variables for plotting
subjBreak <- seq(0,70,by=10)
