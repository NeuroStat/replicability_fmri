####################
#### TITLE:     Calculate within sample size stability with increasing sample sizes: scenario A, B and C!
#### Contents:
####
...
# Set WD
wd <- "/Volumes/2_TB_WD_Elements_10B8_Han/PhD/IMAGENDATA/Data/FreddieFreeloader/Data/FLAME"
setwd(wd)
...
load(paste(wd,'/Scenario6/MatrixOverlapSplitSubj6',sep=''))
	...
	subjA <- data.frame('Size' = rep(seq(10,700,by=10),NRUNSA))
		rm(MatrixOverlap)
load(paste(wd,'/Scenario7/MatrixOverlapSplitSubj7',sep=''))
	OverlapB <- matrix(MatrixOverlap,ncol=1)
	scenB <- data.frame('Scenario' = rep('B',length(OverlapB)))
	subjB <- data.frame('Size' = rep(seq(10,130,by=10),NRUNSB))
		rm(MatrixOverlap)
load(paste(wd,'/Scenario8/MatrixOverlapSplitSubj8',sep=''))
	OverlapC <- matrix(MatrixOverlap,ncol=1)
	scenC <- data.frame('Scenario' = rep('C',length(OverlapC)))
	subjC <- data.frame('Size' = rep(seq(10,60,by=10),NRUNSC))
		rm(MatrixOverlap)
...
# Only take scenario 6, together with the average per sample size.
Scen6 <- AllScen[AllScen$Scenario == 'A',]
Avg <- aggregate(Overlap ~ Size, data = Scen6, FUN = mean)
Scen6$Average <- rep(Avg$Overlap, NRUNSA)

ggplot(Scen6, aes(x=Size,y=Overlap)) +
	...
	geom_hline(yintercept=0.37608886, size = 1.2,linetype = 'longdash') +
	scale_x_discrete(breaks=subjBreak, name="Sample size") +
	scale_y_continuous(name = "Modified Dice index") +
	theme(plot.title = element_text(lineheight=.2, face="bold"),
...
#################
## Scenario 6 with 7 and 8
#################
LAB_JSM <- c('no restrictions','between centra','within centra')

# Take only sample sizes < 200
ID <- AllScen$Size < 200
AllScen200 <- AllScen[ID,]
ggplot(AllScen200, aes(x=factor(Size),y=Overlap))+
	geom_boxplot(aes(color=Scenario), outlier.size = 1.2, lwd=1) +
	scale_color_manual(values = colour, name = "Sampling", labels=LAB_JSM) +
	scale_y_continuous(name = "Modified Dice index") +
			theme(plot.title = element_text(lineheight=.2, face="bold"),
						axis.text=element_text(size=20, face="bold"),
						axis.title=element_text(size=24),
