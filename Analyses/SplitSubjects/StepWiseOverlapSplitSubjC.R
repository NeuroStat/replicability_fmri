####################
#### TITLE:     Calculate within sample size stability with increasing sample sizes: scenario C.
#### Contents: 	
#### 
...
# Scenario C:
	...
	# Furthermore, they are from the same center!
...
# Set WD
wd <- "/Volumes/2_TB_WD_Elements_10B8_Han/PhD/IMAGENDATA/Data/FreddieFreeloader/Data/FLAME/Scenario8"
setwd(wd)
...
## Save objects
save(MatrixOverlap,file=paste(wd,'/MatrixOverlapSplitSubj8',sep=''))
save(PercAct,file=paste(wd,'/PercActSplitSubj8',sep=''))
...
Overlap.tmp <- matrix(MatrixOverlap,ncol=1)
	sampleSize <- rep(seq(10,60,by=10),NRUNS)
Overlap <- data.frame('overlap' = Overlap.tmp, 'size' = sampleSize)

# Variables for plotting
subjBreak <- seq(0,60,by=10)
...
observations <- apply(MatrixOverlap,c(1),function(x){length(na.omit(x))})
	# put in data frame
	obs <- data.frame('count' = observations, 'size' = seq(10,60,by=10))

ggplot(obs, aes(x=factor(size), y=count))+
	geom_bar(stat='identity', colour='#1f78b4',fill='#a6cee3') +
	scale_x_discrete(breaks=seq(0,60,by=10), name="sample size") +
...
ggplot(AvgOverPerc, aes(x=factor(Size),y=Value,group=Type)) + 
	...
	annotate("text", y = .35, x = 1, label = paste('Correlation = ', corr,sep=''),size=5)
