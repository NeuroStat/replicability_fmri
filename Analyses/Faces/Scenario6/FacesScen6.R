####################
#### TITLE:     Calculate within sample size stability with increasing sample sizes: scenario 6 based on FACES data.
#### Contents: 	
#### 
...
## Save objects
save(MatrixOverlap,file=paste(wd,'MatrixOverlapFaceScen6Amy',sep=''))
save(PercAct,file=paste(wd,'PercActFaceScen6Amy',sep=''))

# Or load them in
load(paste(wd,'/MatrixOverlapFaceScen6',sep=''))
load(paste(wd,'/PercActFaceScen6',sep=''))
...
# Matrix were data will get into
MatrixOverlapAmyg <- array(NA,dim=c(NSTEP,NRUNS))

# Vector where we will store the percentage of activated masked voxels
	# As we have two images per step and run, we will take the average of the percentage. 
PercActAmygd <- array(NA,dim=c(NSTEP,NRUNS))
...
for(i in 1:NRUNS){
	...
	for(j in 1:NSTEP){
		...
		#-------------------------------------------------------------------------------------------------------------#
		# Put overlap in matrix
		MatrixOverlapAmyg[j,i] <- round((Vjt)/(Vj + Vt - Vjt),6)
		...
		baseG2 <- sum(maskG2)
		percG2 <- Vt/baseG2
		PercActAmygd[j,i] <- round(mean(c(percG1,percG2)),4)
...
# Transform NaN values to 0
MatrixOverlapAmyg[is.nan(MatrixOverlapAmyg)] <- 0

# Check for missing values (TRUE = missing somewhere)
	complete.cases(t(MatrixOverlapAmyg))
		MissingValues <- function(...){
			any(is.na(...))
		}
	apply(MatrixOverlapAmyg,c(2),MissingValues)


## Prepare matrix
Overlap.tmp <- matrix(MatrixOverlapAmyg,ncol=1)
	sampleSize <- rep(seq(10,700,by=10),NRUNS)
Overlap <- data.frame('overlap' = Overlap.tmp, 'size' = sampleSize)
...
ggplot(AvgOverPerc, aes(x=factor(Size),y=Value,group=Type)) + 
	...
	geom_point(aes(colour=Type),position='identity',size=1.5) +
	scale_x_discrete(breaks=subjBreak, name="Sample size") +
	scale_y_continuous(name='Overlap',limits=c(0, 1)) +
	theme(plot.title = element_text(lineheight=.2, face="bold")) +
	ggtitle('Average overlap and percentage of masked voxels active.')+
	annotate("text", y = .1, x = 60, label = paste('Correlation = ', corr,sep=''),size=5)
