####################
#### TITLE:     Faces data set: unconditional mutual information between two images
#### Contents:
####
#### Source Files: /Volumes/2_TB_WD_Elements_10B8_Han/PhD/IMAGENDATA/Data/FreddieFreeloader/Script/Analyses/Faces/Unconditional
...
for(i in 1:NRUNS){
	...
	for(j in 1:NSTEP){
		...
		imageG1 <- try(readNIfTI(paste(datawd,'/Run_',i,'/Step_',j,'/Group1/stats','/tstat1.nii.gz',sep=''))[,,],silent=TRUE)
			...
			maskG1 <- try(readNIfTI(paste(datawd,'/Run_',i,'/Step_',j,'/Group1','/masked.nii.gz',sep=''))[,,],silent=TRUE)
		...
		imageG2 <- try(readNIfTI(paste(datawd,'/Run_',i,'/Step_',j,'/Group2/stats','/tstat1.nii.gz',sep=''))[,,],silent=TRUE)
			...
			maskG2 <- try(readNIfTI(paste(datawd,'/Run_',i,'/Step_',j,'/Group2','/masked.nii.gz',sep=''))[,,],silent=TRUE)
...
MatrixCorrelation[is.nan(MatrixOverlap)] <- 0

# Check for missing values (TRUE = complete)
	complete.cases(t(MatrixCorrelation))

# Save objects
save(MatrixCorrelation,file=paste(saveobjWD,'/MatrixCorrelationFace',sep=''))
