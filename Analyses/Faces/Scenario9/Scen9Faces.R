...
# Or load them in
load(paste(wd,'/MatrixOverlapAdapFace',sep=''))
load(paste(wd,'/PercActSplitAdapFace',sep=''))
load(paste(wd,'/SignLevelsAdapFace',sep=''))
...
##
###############
### Calculate test-retest reliability in amygdala
###############
##
...
MatOvAdapAmyg <- array(NA,dim=c(NSTEP,NRUNS))

# Vector where we will store the percentage of activated masked voxels
	# As we have two images per step and run, we will take the average of the percentage. 
PercActAdapAmyg <- array(NA,dim=c(NSTEP,NRUNS))


# Vector to store average of percentage activated masked voxels in each step and run
SignLevelsAmyg <- array(NA,dim=c(NSTEP,NRUNS))

# Vector for activation map of first run only
SPMRun1G1Amyg <- array(NA,dim=c(prod(DIM),NSTEP))
SPMRun1G2Amyg <- array(NA,dim=c(prod(DIM),NSTEP))

# What percentage do we want to have on the lower bound?
PercToAchieve <- 0.05


# Adaptive Thresholding
AdapThresholdingAmygd <- function(pvals,DIM,signLevel,idMask,percentageToAch){
	# No adjustment for multiple testing with P-values with Benjamin & Hochberg procedure (built in R function)
	adjPvals <- pvals
	...
	while(percentage < lower | percentage > upper){
		...
		Vt <- sum(idP,na.rm=TRUE)
		percentage <- Vt/maskedVox
		if(index %in% WhileID) print(list('percentage' = percentage,'signLevel' = signLevel))
		}
...
AdapThresholdingAmygd <- function(pvals,DIM,signLevel,idMask,percentageToAch){
	...
	# Lower bound based on percentage
	lower <- percentageToAch-.01
...
for(i in 1:NRUNS){
	...
	for(j in 1:NSTEP){
		...
		# Function AdapThresholdingAmygd: calculates adjusted p-values, then finds correct significance level according to percentage.
		adapG1 <- AdapThresholdingAmygd(pvalsG1,DIM,signLevel,idMaskG1,PercToAchieve)
			signLevel <- adapG1$signLevel
			perc <- adapG1$percentage

		# Image G2
		adapG2 <- AdapThresholdingAmygd(pvalsG2,DIM,signLevel,idMaskG2,PercToAchieve)
			signLevel <- round(mean(c(signLevel,adapG2$signLevel)),6)
			perc <- round(mean(c(perc,adapG2$percentage)),4)
		...
		# Put overlap, percentage and signLevel in matrix
		MatOvAdapAmyg[j,i] <- round((Vjt)/(Vj + Vt - Vjt),6)
		PercActAdapAmyg[j,i] <- perc
		SignLevels[j,i] <- signLevel
		...
		if(i==1){
			...
			# Activation maps
			SPMRun1G1Amyg[,j] <- adapG1$SPM
			SPMRun1G2Amyg[,j] <- adapG2$SPM
		}
...
# Transform NaN values to 0
MatOvAdapAmyg[is.nan(MatOvAdapAmyg)] <- 0




## Prepare matrix
Overlap.tmp <- matrix(MatOvAdapAmyg,ncol=1)
	sampleSize <- rep(seq(10,700,by=10),NRUNS)
Overlap <- data.frame('overlap' = Overlap.tmp, 'size' = sampleSize)
...
# Data frame with overlap and percentage
	perc <- matrix(PercActAdapAmyg,ncol=1)
OverPerc <- data.frame('Value' = rbind(Overlap.tmp,perc),
		'Size' = rep(sampleSize,2), 
		'Type' = c(rep('Overlap',2940),rep('Percentage',2940)))
	OverPerc$Type <- as.factor(OverPerc$Type)
